...
import dev.kstep.mcp.dto.BuildProductContextArgs
...
private const val TOOL_NAME = "build_product_context"
    ...
    ToolSchema(
        properties =
            buildJsonObject {
                ...
                putJsonObject("discipline_type") { put("type", "string") }
            },
        required = listOf("handle", "name", "frame_of_reference_handle", "discipline_type"),
    )

fun registerBuildProductContextTool(
    server: Server,
    store: EntityStore,
) {
    server.addTool(
        name = TOOL_NAME,
        description =
            "Builds a kSTEP AP242 'product_context' entity, referencing an already-built " +
                ...
                "(this entity has no natural id of its own). Referenced later by build_product's " +
                "frame_of_reference_handles (a product needs at least one product_context). Re-building with " +
                "a handle already in the store OVERWRITES the previous entry.",
        inputSchema = INPUT_SCHEMA,
    ) { request ->
        mcpToolCall(TOOL_NAME) {
            val args =
                Json.decodeFromJsonElement<BuildProductContextArgs>(request.arguments ?: JsonObject(emptyMap()))
            ...
            args.disciplineType?.let { requireBoundedString("discipline_type", it) }
            ...
            when (
                val result =
                    productContext {
                        ...
                        disciplineType = args.disciplineType
                    }
            ) {
                is ValidationResult.Invalid -> validationFailedError(result.violations)
                is ValidationResult.Valid -> {
                    val entity = result.value
                    storeOrCapacityError(store, args.handle, EntityStoreEntry.ProductContextEntry(entity)) {
                        CallToolResult(
                            content =
                                listOf(
                                    TextContent(
                                        text =
                                            "Built product_context '${args.handle}': name='${entity.name}', " +
                                                "discipline_type='${entity.disciplineType}'",
                                    ),
                                ),
                            structuredContent =
                                buildJsonObject {
                                    ...
                                    put("discipline_type", entity.disciplineType)
                                    put("entityType", "product_context")
                                },
                        )
                    }
                }
