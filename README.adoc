...
kSTEP applies the pattern already validated by its sibling project,
https://github.com/kuml-dev/kUML[kUML]: expose a domain as a type-safe,
embedded Kotlin DSL with semantic validation and structured errors, so
that both human developers and LLM agents can work in it reliably. The
syntax stays Kotlin (high-resource); only the vocabulary is
domain-specific, enforced by the type system. kSTEP compiles that DSL
down to the neutral, standardized ISO 10303 exchange format.
